...
Der aktive Check ist `scripts/security-check.mjs` und wird plattformübergreifend über `npm run security:check` ausgeführt. Eine Bash-Variante wird nicht gepflegt, damit lokale Windows-Entwicklung und GitHub Actions dieselbe Implementierung verwenden.
